...
output_dir = "./outputs/base_reason/"
...
def main(args):
    ...
    print(f"{COLOR_INFO}Model loaded on device: {device}{COLOR_RESET}")
    
    telemetry_script = os.path.join(os.path.dirname(__file__), "telemetry.sh")
    telemetry_proc = subprocess.Popen(
        ["bash", telemetry_script, output_dir, args.subset_name],
    ...
    for idx, item in enumerate(tqdm(eval_dataset, desc="Evaluating Questions")):
        ...
        predicted_choice_letter, full_decoded_output, inference_time, output_tokens = predict_local(tokenizer, model, messages, device)
